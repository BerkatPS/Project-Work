...
$id = $_SESSION['userId'];
$dataUser = $confg->query("SELECT tbl_absensi_siswa.id AS id_user_absensi , user.id , tbl_absensi_siswa.tanggal ,tbl_absensi_siswa.Hari, user.NIS , user.name , user.KELAS , tbl_absensi_siswa.status, user.poin FROM tbl_absensi_siswa JOIN user ON (tbl_absensi_siswa.id_absensi_siswa = user.id) WHERE user.id = $_SESSION[userId]");
...
<head>
    ...
    <title>APP KESISWAAN - MATA PELAJARAN</title>
</head>
...
<div class="md:flex md:flex-row md:min-h-screen">
        ...
        <div class="container flex justify-end items-center mx-auto">
                ...
                <button type="button" onclick="window.location.href= 'absensi-form'"name="submitabsen" class="flex gap-2 font-medium text-white rounded-md p-2 bg-indigo-500 focus:outline-none hover:shadow-lg shadow-indigo-300/100"><img src="../../icons/plus.png"class="h-6 w-6" alt="" srcset="">Silahkan Absensi</button>
                <div class="py-2"></div>
                <div class="px-6 py-4  border-0 flex relative mb-4 bg-yellow-dark text-yellow-500">
                    ...
                    <span class="inline-block font-bold align-middle mr-8">
                         FORM ABSENSI HANYA BISA DISUBMIT 1 KALI DALAM SEHARI PADA PUKUL 6 PAGI SAMPAI PUKUL 7 PAGI
                    </span>
                    ...
                    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400 rounded-lg" id="example">
                        ...
                        ?>
                            <tr class="text-center relative ">
                                <td class="px-6 py-4 bg-slate-800 font-medium"><?= $row['id_user_absensi'] ?></td>
                                <td class="px-6 py-4 bg-slate-800 font-medium"><?= $row['tanggal'] ?></td>
                                <td class="px-6 py-4 bg-slate-800 font-medium"><?= $row['Hari'] ?></td>
                                <td class="px-6 py-4 bg-slate-800 font-medium text-indigo-500"><?= $row['NIS'] ?></td>
                                <td class="px-6 py-4 bg-slate-800 font-medium"><?= $row['name'] ?></td>
                                <td class="px-6 py-4 bg-slate-800 font-medium"><?= $row['KELAS'] ?></td>
                                ...
                                <td class="px-6 py-4 bg-slate-800 font-medium">
                                <?php
                                    $update = "UPDATE user SET poin = ";
                                    if($row['status'] == 'HADIR'){
                                        $update;
                                        echo'<span class="text-green-400">'.$row['poin'].'</span>';
                                    }else if($row['status'] == 'TERLAMBAT'){
                                        $row['poin'] = $row['poin'] + 5;
                                        echo'<span class="text-red-600 p-2">+'.$row['poin'].'</span>';    
                                    }else if($row['status'] == 'ALPHA'){
                                        $row['poin'] = $row['poin'] + 10;
                                        echo'<span class="text-red-600 p-2">+'.$row['poin'].'</span>';    
                                    }else{
                                        $row['poin'] = $row['poin'] + 0;
                                        echo'<span class="text-green-400">'.$row['poin'].'</span>';
